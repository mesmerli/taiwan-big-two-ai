...
class BaseLLMAI extends AICharacter {
    constructor(gameLogic, name, avatar = "✨") {
        ...
        if (this.name === "Diana") {
            if (!AppStorage.getItem(this.settingsKey) && AppStorage.getItem('ai_settings_Diana')) {
    ...
    loadSettings() {
        ...
        try {
            const memory = AppStorage.getItem(this.memoryKey);
            if (memory) {
    ...
    saveMemory() {
        try {
            AppStorage.setItem(this.memoryKey, JSON.stringify({

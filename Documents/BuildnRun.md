...
*   **建置**：
    ...
    > - **測試商店版**：請執行 `npx cross-env BUILD_TARGET=STORE npm run pack`，再開啟 `dist/win-unpacked/Taiwan Big2 AI.exe`。
    > - **測試側載版**：請執行 `npx cross-env BUILD_TARGET=GITHUB npm run pack`，再開啟 `dist/win-unpacked/Taiwan Big2 AI.exe`。
